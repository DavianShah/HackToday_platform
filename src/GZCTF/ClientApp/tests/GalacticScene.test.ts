...
test('only genuine positive ordinary deltas produce solve attacks', () => {
  const events = positiveSolveEvents(new Map([[1, 150], [2, -20], [3, 0], [4, NaN], [5, 250]]), new Set([1, 2, 3, 4]), 'poll')
  assert.deepEqual(events.map(e => [e.teamId, e.delta]), [[1, 150]])
})
...
test('cinematic starts at active event, deduplicates and interrupts solves, then recovers', () => {
  ...
  const blood = announcementScene({ key: 'blood', kind: 'firstBlood', title: 'First Blood', sound: 'firstBlood', popupDelay: 5000, duration: 8600 }, false, false)!
...
test('visual backlog is bounded and coalesces score facts per team', () => {
  ...
  for (let i = 0; i < 50; i++) scheduler.push({ key: String(i), kind: 'correct', teamId: i % 15, delta: 1, duration: 1500 }, 1)
