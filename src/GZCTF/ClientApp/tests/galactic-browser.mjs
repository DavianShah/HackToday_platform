...
const checkLayout = async (width, height) => {
  ...
  const size = await evaluate(`({ canvas: document.querySelectorAll('canvas').length,
    ...
    timer: [...document.querySelectorAll('header strong')].at(-1).getBoundingClientRect().toJSON() })`)
...
try {
  ...
  await invoke('solve')
  await pause(250)
  assert.match(await statuses(), /\+150/)
  await pause(1600)
  await invoke('firstBlood')
  await pause(300)
  assert.match(await statuses(), /Priority strike/i)
  assert.doesNotMatch(await statuses(), /FIRST BLOOD/)
  const beforeTimer = await evaluate('[...document.querySelectorAll("header strong")].at(-1).innerText')
  await pause(4850)
  assert.match(await statuses(), /FIRST BLOOD/)
  const afterTimer = await evaluate('[...document.querySelectorAll("header strong")].at(-1).innerText')
  ...
  for (const [action, label, duration] of [
    ...
    await invoke(action)
    await pause(action === 'hint' ? 2300 : 300)
    assert.match(await statuses(), new RegExp(label))
    await pause(duration - (action === 'hint' ? 2300 : 300))
  }
  ...
  await pause(200)
  assert.match(await statuses(), /\+100/)
  await pause(1600)
  ...
  const timer1 = await evaluate('[...document.querySelectorAll("header strong")].at(-1).innerText')
  await pause(4300)
  assert.ok(polls >= 3)
  assert.notEqual(timer1, await evaluate('[...document.querySelectorAll("header strong")].at(-1).innerText'))
