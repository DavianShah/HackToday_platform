...
const validTime = (value?: number): value is number =>
  typeof value === 'number' && Number.isFinite(value) && Math.abs(value) <= 8640000000000000

const eventTime = (createdAt?: number) =>
  validTime(createdAt) ? clockFormatter.format(new Date(createdAt)) : '--:--:--'

export const LiveEventStream: FC<{ events: LiveScoreboardEventModel[]; frozen?: boolean }> = ({ events, frozen }) => (
  <aside className={`${classes.panel} ${classes.eventPanel}`}>
    <header className={classes.panelHead}>
      <div>
        <span className={classes.eyebrow}>Galactic transmission</span>
        <h2 className={classes.panelTitle}>Recent events</h2>
      </div>
      <i className={classes.signal} aria-hidden />
    </header>
    {frozen ? (
      <div className={classes.sealedRows}>
        <div>
          <strong>Feed sealed</strong>
          <span>Event identities are hidden while standings are frozen.</span>
        </div>
      </div>
    ) : (
      <div className={classes.eventRows}>
        {events.slice(0, 8).map((event) => {
          ...
          return (
            <article className={`${classes.eventRow} ${color}`} key={event.id ?? `${event.createdAt}-${event.message}`}>
              <time
                className={classes.eventTime}
                dateTime={validTime(event.createdAt) ? new Date(event.createdAt).toISOString() : undefined}
              >
                {eventTime(event.createdAt)}
              </time>
              <div className={classes.eventBody}>
                <b className={classes.eventBadge}>{label}</b>
                <p className={classes.eventMessage}>{event.message}</p>
              </div>
            </article>
          )
        })}
