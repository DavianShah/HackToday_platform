...
export const LiveScoreboardStage: FC<{
  ...
  const sceneEvent = useSceneDirector({ announcement: presentation.announcement, deltas: presentation.scoreDeltas,
    changed: presentation.changedTeams, frozen: Boolean(state?.scoreboardFrozen), preview: Boolean(preview),
  ...
  return <main className={`${classes.stage} ${overtime ? classes.stageOvertime : ''}`}>
    <Suspense fallback={null}><GalacticScene overtime={overtime} teams={teams} frozen={frozen} event={sceneEvent} spinPhase={presentation.spinPhase}
      highlighted={new Set(presentation.rankChanges.keys())} categoryCount={new Set([...(state.speedrunState?.remainingCategories ?? []), ...(state.speedrunState?.usedCategories ?? [])]).size} /></Suspense>
      ...
      <div className={classes.content}>
        <div className={classes.mainGrid}>
          ...
          <LiveCenterArena round={round} remaining={state.speedrunState?.remainingCategories ?? []}
            ...
            bloodTeams={frozen ? new Set() : new Set([...presentation.bloodAttackTeams, ...(sceneEvent?.teamId ? [sceneEvent.teamId] : [])])}
    ...
    {sceneEvent && !frozen && (sceneEvent.teamId || sceneEvent.teamName) && <div className={classes.sceneCallout} role="status">
      <span>{sceneEvent.kind === 'wrong' ? 'Preview ? Target deflected' : sceneEvent.kind === 'firstBlood' ? 'Priority strike' : 'Verified strike'}</span>
      <strong>{teams.find(team => team.id === sceneEvent.teamId)?.name ?? sceneEvent.teamName ?? 'Incoming transmission'}</strong>
