...
const PreviewStage: FC<{ gameId: number; config: LiveScoreboardConfigModel }> = ({ gameId, config }) => {
  ...
  return (
    ...
    <LiveScoreboardStage
      ...
      />
      {deckOpen ? (
        <section className={classes.controlDeck} aria-label="Live scoreboard preview controls">
          <header className={classes.deckHeader}>
            <div className={classes.deckIdentity}>
              <i />
              <div>
                <span>Admin preview</span>
                <b>Live stage controls</b>
              </div>
            </div>
          ...
          <div className={classes.deckBody}>
            ...
            <div className={classes.triggerGrid}>
              <button onClick={preview.spin}>Spin category</button>
              <button onClick={preview.start}>Start round</button>
              <button onClick={preview.solve}>Solve +150</button>
              <button onClick={preview.firstBlood}>First Blood</button>
              <button onClick={preview.secondBlood}>Second Blood</button>
              <button onClick={preview.thirdBlood}>Third Blood</button>
              <button onClick={preview.hint}>Release hint</button>
              <button onClick={preview.reminder}>60s reminder</button>
              <button onClick={preview.countdown}>10s countdown</button>
              <button onClick={preview.overtime}>Overtime</button>
              <button onClick={preview.finish}>Finish round</button>
              <button onClick={preview.wrongSubmit}>Wrong submit</button>
              <button onClick={preview.toggleFreeze}>
                {preview.state.scoreboardFrozen ? 'Unfreeze standings' : 'Freeze standings'}
              </button>
            ...
            <div className={classes.showcase}>
              <div className={classes.showcaseStatus}>
                <span>Showcase</span>
                <b>{preview.showcaseStep}</b>
              </div>
              <div className={classes.showcaseActions}>
                <button disabled={preview.showcaseRunning} onClick={preview.runShowcase}>
                  Play sequence
                </button>
                <button disabled={!preview.showcaseRunning} onClick={preview.stopShowcase}>
                  Stop
                </button>
              </div>
            </div>
        ...
        <button className={classes.deckClosed} onClick={() => setDeckOpen(true)}>
...
const LiveScoreboardPreviewPage: FC = () => {
  ...
  return (
    <WithRole requiredRole={Role.Admin}>
      {config ? (
        <PreviewStage gameId={gameId} config={config} />
      ) : (
        <div className={classes.loading}>Loading live stage preview</div>
      )}
    </WithRole>
  )
}
